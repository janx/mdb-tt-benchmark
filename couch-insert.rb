...
(begin_cat_id .. end_cat_id).each do |cat_id| # 20000 × 55 = 1,100,000
    ...
    count.times do
        ...
        source_url = "http://github.com/janx"
        status = 1
        ...
        doc = <<-JSON
          {"ID":"#{id}", "Title":"#{title}", "CategoryID":"#{cat_id}", "UserID":"#{user_id}", "Tags":"#{tags.join(" ")}", "Source":"#{source}", "SourceUrl":"#{source_url}", "Status":"#{status}"}
        JSON
        server.put("/test/#{id}", doc)
